...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        static struct option long_options[] = {
            ...
            {"verbose",            0, 0, 'v'}, /* talk about program execution */
            {"absolute-threshold", 0, 0, 'a'}, /* use threshold value as provided */
            {"iter-threshold",     0, 0, 'T'}, /* use threshold value as provided */
            ...
            {"adjust-gray",        0, 0, 'g'}, /* use T1 and T2 as percentages of used values */
            ...
            case 't':
                if (optarg) {
                    ...
                    if (thresh < 0.0 || 100.0 < thresh) {
                        thresh = THRESHOLD;
                        if (flags & VERBOSE) {
                            fprintf(stderr, "ignoring --threshold=%s\n", optarg);
                        }
                    }
                    if (flags & DEBUG_OUTPUT) {
                        fprintf(stderr, "thresh = %f (default: %f)\n", thresh, THRESHOLD);
    ...
    if (flags & DEBUG_OUTPUT) {
        ...
        print_lum_key(lt, stderr);
        fprintf(stderr, "\n");
        fprintf(stderr, "charset    = ");
        print_cs_key(charset, stderr);
        fprintf(stderr, "\n");
